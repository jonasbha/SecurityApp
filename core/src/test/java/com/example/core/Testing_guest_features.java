...
public class Testing_guest_features {
    ...
    public void guest_can_read_dialog_in_a_course_when_valid_PIN_is_given() throws Exception {
        Message studMsg = new Message("Hello from student", dialog, false);
        student.sendMessage(studMsg);
        Message teachMsg = new Message("Hello from teacher", dialog);
        student.sendMessage(teachMsg);

        assertEquals("Hello", guest.getCourse(course, 123).getDialogs().get(0).getMessages().peek().getText());
        assertEquals("Hello", guest.getCourse(course, 123).getDialogs().get(0).getMessages().peek().getText());
    }
    ...
    public void guest_can_report_inappropriate_messages_from_student() throws Exception {
        Message msg = new Message("You are fat.", dialog, true);
        student.sendMessage(msg);

        Report report = guest.reportMessage(guest.getCourse(course, 123).getDialogs().get(0).getMessages().peek(),
                "rude");
